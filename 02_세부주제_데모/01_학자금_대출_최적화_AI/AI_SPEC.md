...
지식은 기존 SLOW 제품 용어와 계산 의미에 한정한다. 세부 대출 정책 FAQ·출처 기반 검색·실시간 웹 검색·챗봇의 입력 변경·음성·대화 영구 저장·외부 배포는 이번 구현에 포함하지 않는다. 삭제된 다른 계획이나 이 범위 밖의 항목은 자동 후속 작업으로 간주하지 않는다.
