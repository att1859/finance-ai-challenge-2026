...
시각 문법은 KPI 카드 묶음보다 입력 행, 선택 그룹, 시간축 비교 선, 접근 가능한 표, 자금 원장을 우선한다. 숫자는 독립된 성과가 아니라 희망 기준, 현재 기준, 계산 가정, 대출유형과 함께 읽힌다. 균형안 v1은 시나리오의 최초 보기일 뿐 추천 시나리오가 아니며, 사용자가 바꾸는 시나리오와 대출 구성이 모든 상세와 위험 계산의 중심이다.
...
기본 컨테이너는 최대 1180px이며 데스크톱 좌우에 28px 안쪽 여백을 둔다. 첫 화면은 SLOW 메인 제목과 다섯 개의 좌우 교차 설명 행으로 구성하고, 입력 폼은 250px 설명 열과 가변 입력 열을 짝지은 행 기반 구조다. 결과는 시나리오 선택, 통합 비교, 선택 상세, 자금 원장, 스트레스, 출처가 하나의 세로 판단 흐름을 만든다.
...
### Input Mode & Deferred Loan Choices

입력 방식은 두 개의 72px 행으로 제시하고 선택된 입력 방식은 남색 면으로 반전한다. 최초 입력에는 실제 납부 등록금과 대출 없이 낼 등록금을 분리하되, 대출상품·생활비 대출·상환기간·특례·기존 대출 선택 면은 두지 않고 결과로 미룬다. 계산 금리와 원금균등 선택은 어느 단계에서도 입력받지 않는다.

결과의 대출 구성은 기존 선택 면과 규칙선 문법을 재사용해 용도별 상품·금액·자격을 같은 라디오 그룹에 둔다. 생활비 대출, 일반 상환기간, 현재 판정에 필요한 자격과 기존 대출은 선택한 구성에 필요한 경우에만 이어지며 풀대출 상한은 추천과 분리된 접이식 참고 정보로 둔다.

결과 제어 변경은 별도 저장 버튼 없이 같은 위치에서 재계산하고, 재렌더 뒤 포커스를 원래 제어로 돌려보낸다. 시각 결과와 스크린리더 상태 알림은 자격·근로시간·실행액·상환값이 함께 갱신됐음을 같은 동작으로 전달한다.

### Scenario Selector & Time Comparison

비교 대상 A·B는 드롭다운으로 선택하고 옆에 내 시나리오 추가 행동을 둔다. 지표는 그래프 제목 옆 SEED segmented-control로 선택하며 좁은 화면에서는 제목 아래에 배치한다. 기본 조합은 최소대출안과 균형안 v1이다. 그래프에는 두 선만 표시하고 남색 실선·근거 파랑 파선·경계 장미 점선을 시나리오 ID에 고정한다. 지표는 월 생활비 여력·상환 부담·남은 대출잔액 중 하나다. 취업 지연은 SEED 세그먼트, 초봉 감소는 간결한 체크 제어로 그래프 위에 두며 추가 조건은 접는다. 범례는 그래프 중앙에 정렬하고 첫 점은 축에서 안쪽으로 띄운다. 조회는 점 클릭 고정·이전/다음·방향키를 제공하며 두 값과 차이·월별 표는 그래프 아래에 둔다. 그래프 아래 두 시나리오 라디오가 상세만 바꾼다.

### Selected Detail & Funding Ledger

선택 상세는 기존 남색 원장 면에서 재학 중 월평균 생활비·근로시간, 졸업 잔액, 취업 첫해 월평균 상환·생활비를 먼저 표시한다. 긴 내역은 대출 구성 및 상환 일정, 자금 계산 내역, 계산 가정 및 출처로 접는다. 핵심 금액에는 기준 시기와 월평균 환산 제한을 붙인다.

추천 설명은 남색 원장 내부 규칙선 구획에서 근로시간 효과와 추천 이유를 먼저, 자격 확인에 필요한 정보와 입력 위치를 다음, 잔액·변동금리·생활비 부족 주의를 마지막에 둔다. 현재 기준 10년의 불변 가정은 10년 비교 구역에 모은다. 추천·조건 확인은 기존 라임 표식을, 주의 문장은 기존 호박색을 사용하며 내부 추천 점수는 표시하지 않는다.
...
변경한 버튼·세그먼트는 설치된 @seed-design/css 레시피를 사용하고 기존 종이·남색·파랑 토큰에 매핑한다. 선택 세그먼트는 한 개의 인디케이터로 구분하며 기본 브라우저 버튼 테두리와 배경을 제거한다. 사용자 시나리오는 native dialog에서 명시적으로 저장하며 이름·근로시간, 생활비 학기 금액, 상품·기간 순서로 구성한다. 입력은 +/- 5만 원 행동과 직접 숫자 입력을 함께 제공하고 오류는 해당 필드와 알림에 표시한다. 모바일 독립 행동은 44px 이상, 학기별 입력은 한 열로 전환한다.
