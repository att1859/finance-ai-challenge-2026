...
시각 문법은 KPI 카드 묶음보다 입력 행, 선택 그룹, 통합 비교 막대, 접근 가능한 표, 자금 원장을 우선한다. 숫자는 독립된 성과가 아니라 희망 기준, 현재 기준, 계산 가정, 대출유형과 함께 읽힌다. 균형안 v1은 시나리오의 최초 보기일 뿐 추천 시나리오가 아니며, 사용자가 바꾸는 시나리오와 대출 구성이 모든 상세와 위험 계산의 중심이다.
...
- **Selection Lime** (lime-selection): 선택된 시나리오, CTA 화살표, 단계 번호, 비교 막대와 추천·조건 확인 표식처럼 현재 행동·선택·규칙 결과에 사용한다.
...
세 시나리오는 하나의 라디오 그룹 안에서 동일한 크기와 문장 구조로 제시한다. 각 선택지는 현재 근로시간에서 파생한 주당 시간과 감소량을 함께 보여준다. 균형안 v1의 “기본 보기”는 최초 상태 설명이고, 현재 사용자가 고른 안에만 “내 선택”과 라임 안쪽 링이 붙는다. 선택 변화는 비교, 추천 대출 구성, 상세, 자금 원장, 스트레스에 즉시 반영되고 스크린리더 상태 메시지로도 안내한다.
...
선택 상세는 남색 원장 면에서 네 핵심 지표, 용도별 상품·원금·자격, 상품별 상환값과 현재 기준 10년 결과를 이어 보여준다. 일반 상환은 거치 중 최대 월이자·상환 시작일·원리금균등 월 납입액, 취업 후 상환은 연간 예상 의무상환액과 `월평균 환산액(참고)`로 단위를 구분한다. 학기별 실행과 계산 순서, 적용 금리 유형·효력일·확인일·공식 출처는 같은 면의 접이식 원장으로 확인한다. 이어지는 밝은 자금 원장은 졸업기간을 학기·개월로 변환하고 실제 납부 등록금과 희망 생활비를 합산한 뒤 대출 없이 낼 등록금, 예상 실수령 근로소득과 등록금·생활비 대출을 각각 차감한다.
