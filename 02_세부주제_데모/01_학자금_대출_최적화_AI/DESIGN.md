...
입력 방식은 두 개의 72px 행으로, 대출유형은 두 개의 96px 선택 면으로 제시한다. 선택된 입력 방식은 남색 면으로 반전하고, 선택된 대출유형은 2px 남색 경계와 라임 중심의 라디오로 표시한다. 일반 상환은 거치·상환기간만 입력하고 원리금균등 기준을 설명하며, 취업 후 상환은 연간 의무상환 정책 안내를 조건부로 보여준다. 계산 금리와 원금균등 선택은 입력받지 않는다.
...
선택 상세는 남색 원장 면에서 네 핵심 지표, 현재 대비 근로시간 감소량, 대출유형별 상환값, 계산 가정, 인접 시나리오 차이를 이어 보여준다. 일반 상환은 원리금균등 월 납입액, 취업 후 상환은 연간 예상 의무상환액과 `월평균 환산액(참고)`로 단위를 구분한다. 이어지는 밝은 자금 원장은 졸업기간을 학기·개월로 변환하고 등록금과 희망 생활비를 합산한 뒤 주휴수당·간편 차감이 반영된 예상 실수령 근로소득과 등록금·생활비 대출을 각각 차감한다.
