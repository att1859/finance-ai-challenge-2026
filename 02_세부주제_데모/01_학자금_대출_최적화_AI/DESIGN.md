...
- 확정 지원금, 졸업기간, 대출유형, 공식 경계를 결과 가까이 드러내는 증거 중심 태도
...
- **Evidence Blue** (evidence-blue): 확정 지원금·근로·대출 차감처럼 계산 항목의 정보 역할을 표시한다.
- **Boundary Rose** (boundary-rose): 입력 오류, 계산 불가, 부족 상태를 나타낸다.
- **Caution Amber** (caution-amber): 후보 금액이 계산에 반영되지 않았음을 알리는 주의 정보에 사용한다.
...
기본 컨테이너는 최대 1180px이며 데스크톱 좌우에 28px 안쪽 여백을 둔다. 첫 화면은 약속과 세 비교축 원장을 1.08 대 0.92 비율로 나누고, 입력 폼은 250px 설명 열과 가변 입력 열을 짝지은 행 기반 구조다. 결과는 시나리오 선택, 통합 비교, 선택 상세, 자금 원장, 지원 정보, 스트레스, 출처가 하나의 세로 판단 흐름을 만든다.
...
입력은 6px, 버튼은 7px, 선택 제어는 8px, 비교 면은 12px 반경을 사용한다. 핵심 남색 원장은 좌상·우상·우하를 크게 둥글리고 좌하를 4px로 접어 문서 같은 방향성을 만든다. 칩과 상태 카운트는 pill, 카탈로그 상태는 작은 3px 표식을 사용한다. 대부분의 경계는 1px이며 선택과 총액 시작선만 2–3px로 강화한다.
...
- **Outline:** 투명 면과 남색 경계로 카탈로그 펼치기 같은 보조 행동을 제공한다.
- **Hover / Focus:** 호버는 1px 상승하고 색·경계만 바꾼다. 포커스는 3px 파란 외곽선과 3px 오프셋을 사용한다.

### Chips

- **Style:** 특별자격과 필터는 최소 42–44px 높이의 pill을 사용한다. 상태 배지는 3px 모서리의 작은 직사각형으로 목록 안에 머문다.
- **State:** 선택 전에는 흰 면과 중립선, 선택 후에는 남색 면 또는 라임 표식으로 즉시 반전한다.
...
세 시나리오는 하나의 라디오 그룹 안에서 동일한 크기와 문장 구조로 제시한다. 균형형의 “기본 보기”는 최초 상태 설명이고, 현재 사용자가 고른 안에만 “내 선택”과 라임 안쪽 링이 붙는다. 선택 변화는 비교, 상세, 자금 원장, 스트레스에 즉시 반영되고 스크린리더 상태 메시지로도 안내한다.

### Grouped Comparison

대학 시절 월 생활비 여력, 주당 근로시간, 상환 후 월 생활비 여력을 하나의 figure 안에서 비교한다. 각 지표 안에서만 막대 길이를 비교하고 희망·현재 기준선을 함께 표시한다. 동일 내용을 표로도 제공해 단위와 정확한 값을 보존한다.

### Selected Detail & Funding Ledger

선택 상세는 남색 원장 면에서 네 핵심 지표, 대출유형별 상환값, 계산 가정, 인접 시나리오 차이를 이어 보여준다. 이어지는 밝은 자금 원장은 졸업기간을 학기·개월로 변환하고 등록금과 희망 생활비를 합산한 뒤 확정 생활비성 지원금, 근로소득, 신규 대출만 차감한다.

### Informational Catalogue, Stress & Sources

지원사업 카탈로그는 후보 상태·이유·금액·확인일·공식 링크를 행으로 제공하며 후보 금액이 계산에 미반영임을 명시한다. 스트레스 제어는 선택안만 다시 계산하고 기준 대비 변화량을 보여준다. 출처 영역은 두 대출유형의 공식 정보와 계산에서 제외한 가정을 같은 레벨로 공개한다.
...
- **Do** 학기당 실제 납부 등록금과 수혜가 확정된 생활비성 지원금만 계산에 반영한다.
...
- **Don't** 지원사업 후보 금액을 확정 지원금처럼 차감하거나 카탈로그를 개인 수혜 결과처럼 표현한다.
