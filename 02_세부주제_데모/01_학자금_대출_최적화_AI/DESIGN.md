...
우측 하단에 210×136px 고정 봇 카드를 두고, 얼굴 아이콘·이름 아래 한 문장의 안내를 두 줄로 표시한다. 클릭하면 380px 설명 패널이 펼쳐지고 카드는 높이 54px 버튼으로 접힌다. 종이색·남색·파랑과 기존 서체를 유지하고 SEED 버튼을 재사용한다. 작은 얼굴 아이콘은 코드로 그린다. 그림자·그라데이션·AI 스파클은 추가하지 않는다.
