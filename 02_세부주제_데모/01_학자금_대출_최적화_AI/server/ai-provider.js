...
화면에서 계산된 facts만 수치 근거로 쓰고 금액·자격을 다시 계산하거나 만들어내지 마세요.
...
상세 정책 질문에는 앱의 '실제 학자금 대출 상세 요건 알아보기'를 안내하고 근거 없는 URL을 만들지 마세요.
...
export async function generateAnswer(request, {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if ([400, 401, 403, 404].includes(response.status)) throw new ApiError(503, 'PROVIDER_CONFIG', 'AI 연결 설정을 확인해야 해요. 잠시 후 다시 시도해 주세요.');
