...
- `calculateAllScenarios(profile, stress, policySnapshot)`: 생활비 학기·누적 한도와 0.5시간 경계를 적용한 최소대출안·균형안 v1·최대활용안을 만들고 중복 결과를 제거한다.
...
입력 프로필은 현재 주당 근로시간과 `workTaxPreset`만 보관하고 계산 금리와 상환방식은 보관하지 않는다. 시나리오 계층은 현재 근로시간부터 0시간까지 0.5시간 후보를 실제 근로소득으로 계산해 최소대출안·균형안 v1·최대활용안을 만들고, 같은 결과는 한 번만 반환한다. 각 시나리오는 `workHoursReduced`, `workIncomeBreakdown`, `tuitionFunding`, 학기별 `livingLoan`, `unmetLivingGap`, 등록금·생활비 실행분과 상품별 상환 결과를 함께 반환한다. 월 생활비 비교에는 일반 월 납입액과 취업후 연간 예상액의 월평균 환산액 합계를 사용하되 두 법적 상환 단위는 결과 계약에서 합치지 않는다. 각 실행분에는 자격과 세 특례 판정이 연결되고 전체 계획은 네 상품 조합의 자격 결과와 추천과 분리된 풀대출 상한 보기를 함께 반환한다.
