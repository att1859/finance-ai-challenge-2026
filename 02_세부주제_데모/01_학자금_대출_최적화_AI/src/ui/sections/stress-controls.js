...
export function renderStressControls(state, scenario) {
  ...
  return `<section class="stress-section" aria-labelledby="stress-title"><div class="section-heading compact"><h3 id="stress-title">계획이 달라져도 감당할 수 있는지 확인해 보세요.</h3><p>조건은 선택한 ${scenario.name}에 바로 반영됩니다.</p></div>
    ...
    <div class="stress-result ${changed?'is-changed':''}"><div><span>선택안의 대학 생활비 여력</span><strong>${money(scenario.possibleCollegeSpend,1)}</strong><small>기준 대비 ${signedMoney(scenario.possibleCollegeSpend-baseline.possibleCollegeSpend)}</small></div><div><span>${state.profile.loanType==='income-contingent'?'연간 예상 의무상환액':'월 원리금균등 납입액'}</span><strong>${state.profile.loanType==='income-contingent'?money(scenario.loan.annualMandatoryRepayment,1):money(scenario.loan.monthlyScheduledPayment,1)}</strong><small>기준 대비 ${signedMoney((state.profile.loanType==='income-contingent'?scenario.loan.annualMandatoryRepayment-baseline.loan.annualMandatoryRepayment:scenario.loan.monthlyScheduledPayment-baseline.loan.monthlyScheduledPayment))}</small></div><div><span>상환 후 월 생활비 여력</span><strong>${money(scenario.possibleCareerSpend,1)}</strong><small>기준 대비 ${signedMoney(scenario.possibleCareerSpend-baseline.possibleCareerSpend)}</small></div>${changed?`<button type="button" data-action="reset-stress">${icon('reset')} 위험 조건 초기화</button>`:''}</div>
  </section>`;
}
