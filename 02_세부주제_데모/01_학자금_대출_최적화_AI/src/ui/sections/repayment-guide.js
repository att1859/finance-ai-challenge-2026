...
import { primaryButton } from '../shared/seed-controls.js';
...
export function renderRepaymentGuide() {
 ...
 <p class="guide-eyebrow">계산이 끝났어요</p><h2 id="result-title">그래프를 보기 전,<br>갚는 방식만 알아볼까요?</h2>
 ...
 <article><span class="guide-tag">변동금리 · 현재 연 ${incomeContingent.interest.annualRate}%</span><h3>취업 후 상환 <small>ICL</small></h3><p>연간 소득이 기준을 넘으면 <strong>소득에 따라</strong> 갚아요.</p><p>금리가 나중에 바뀔 수 있어 총이자와 갚는 기간도 달라질 수 있어요. 취업 자체가 상환 시작 기준은 아니에요.</p></article>
 </div>
 <p class="guide-assumption">아래 계산은 현재 금리가 유지된다고 가정해요. ICL의 미래 금리 변화는 예측하지 않아요.</p>
 <p>처음에는 등록금·생활비 모두 <strong>일반 상환</strong>으로 비교해요. 그래프 아래에서 각 안의 상품을 바꿀 수 있어요.</p>
